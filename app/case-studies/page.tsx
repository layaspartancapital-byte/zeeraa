  ...
  const schema = [
    {
      ...
      description: "See how Zeeraa drives measurable eCommerce growth across Amazon, Shopify, TikTok Shop, and more.",
      ...
      mainEntity: {
        ...
        itemListElement: studies.map((study, i) => ({
          "@type": "ListItem", position: i + 1, name: study.title,
        ...
        <div className="max-w-7xl mx-auto">
          <nav className="mb-8 text-sm text-text-muted" aria-label="Breadcrumb">
            <ol className="flex items-center gap-2">
              ...
              <li>/</li>
              <li className="text-text-primary">Case Studies</li>
            </ol>
          </nav>

          <div className="text-center mb-16">
            <h1 className="font-syne text-4xl md:text-6xl font-bold text-text-primary">Case Studies</h1>
            <p className="mt-4 text-text-muted text-lg max-w-2xl mx-auto">
              Real results for real brands. See how Zeeraa drives measurable eCommerce growth across every platform.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {studies.map((study) => (
              <Link
                ...
                className="group rounded-xl border border-border bg-white p-8 shadow-card hover:shadow-card-hover hover:border-primary hover:-translate-y-0.5 transition-all duration-300"
              >
                <span className="inline-block px-3 py-1 rounded-full text-xs font-medium bg-primary-light text-primary mb-4">
                  {study.industry}
                </span>
                <h2 className="font-syne text-xl font-bold text-text-primary mb-2 group-hover:text-primary transition-colors">
                  {study.client}
                </h2>
                <div className="mb-4">
                  <span className="text-primary font-syne font-bold text-3xl">{study.results}</span>
                  <span className="block text-text-muted text-sm mt-1">{study.metric}</span>
                </div>
                ...
                <p className="text-text-muted text-sm leading-relaxed">{study.excerpt}</p>
